...
export default function ListingDetail(props){
   ...
   const reserveListing = () => {
       ...
       axios.post('http://206.189.249.190/v1/reservations', body).then(res => {
           console.log('success!');
   ...
   useEffect(() => {
       const fetchData = async () => {
           const listingData = () => axios.get(`http://206.189.249.190/v1/listings/${params.id}`).then(res => {
               setListing(res.data);
               console.log(res.data.reservationId)
               reservationData(res.data.reservationId);
           })
           const reservationData = (reservationId) => axios.get(`http://206.189.249.190/v1/reservations/${reservationId}`).then(res => {
           ...
           const imageData = () => axios.get(`http://206.189.249.190/v1/images/listing/${params.id}`).then(res => {
